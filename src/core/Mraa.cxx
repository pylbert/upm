...
#define MraaThrow(failure, protocol, fullstring) throw std::runtime_error("Failed to initialize " + \
...
Mraa::Mraa(const std::string &init_string) : Mraa()
{
    DEBUG_MSG("XXX");
                    ...
                    for(size_t i = 2; i < params.size(); ++i)
                    {
                        ...
                        else if (_mraa_OutputMode.find(params[i]) != _mraa_OutputMode.end())
                        {
                            if (_gpio.back()->outputMode(_mraa_OutputMode[params[i]]) != mraa::SUCCESS)
                                MraaThrow("outputMode(" + params[i] + ") failed.", "GPIO", proto_str);
                        }
                        /* Empty chunk? Skip it */
                    ...
                    for(size_t i = 2; i < params.size(); ++i)
                    {
                        ...
                        if (_mraa_I2cModes.find(params[i]) != _mraa_I2cModes.end())
                        {
                            if (_i2c.back()->frequency(_mraa_I2cModes[params[i]]) != mraa::SUCCESS)
                                MraaThrow("frequency(" + params[i] + ") failed.", "I2C", proto_str);
                        }
                    }
                    break;
                }
