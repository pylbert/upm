...
void Gadget::initFromJsonLibDesc(std::string path_to_json_file)
{
    ...
    *_json = _json->parse(json_str.c_str(), nullptr, false);
    ...
    if (!_child_json_deserializers.empty())
    {
        ...
        for (std::map<t_getJson, Gadget*>::const_iterator it = _child_json_deserializers.begin();
...
std::string Gadget::JsonDefinition ()
{
    std::stringstream ss;
    ss << "{" << std::endl
       << "  \"name\" : \"" << Name() << "\"," << std::endl
...
std::string Gadget::DerivedClassName() const
{
    int status = 0;
    char *name_demangled;
...
std::string Gadget::LibraryAbsolutePath() const
{
    Dl_info info;
    const std::type_info &this_type = typeid(*this);
...
std::string Gadget::LibraryBaseName() const
{
...
std::string Gadget::LibraryJsonFilename() const
{
...
std::string Gadget::DataDirectory() const
{
...
std::string Gadget::LibraryJsonRaw() const
{
...
std::string Gadget::LibraryVersion() const
{
    return ::LibraryVersion();
}

void Gadget::AddValueSerializer (Gadget * instance, t_getJson method)
{ _child_value_serializers[method] = instance; }

void Gadget::AddJsonDeserializer(Gadget * instance, t_getJson method)
