...
std::string Actuator::LibraryJsonCommands()
{
    std::stringstream ss;
...
void Actuator::AddCommand (std::string source, std::string unit)
{
...
std::map <std::string, std::string> const & Actuator::CommandMap () const
{ return _sinks_2_units;}

std::vector <std::string> const & Actuator::Commands ()
{
...
std::vector <std::string> const & Actuator::Units () const
{ return _units; }

std::string const Actuator::Unit (std::string source) const
{
...
bool Actuator::HasCommand (std::string source)
{
    return std::find(Commands().begin(), Commands().end(), source) != Commands().end();
}

bool Actuator::HasUnit (std::string unit) const
{
    return std::find(Units().begin(), Units().end(), unit) != Units().end();
}

std::string Actuator::JsonDefinition ()
{
    std::stringstream ss;
...
void Actuator::JsonCommands(std::string commands)
{
    std::stringstream ss;
    ss << "{" << std::endl;
